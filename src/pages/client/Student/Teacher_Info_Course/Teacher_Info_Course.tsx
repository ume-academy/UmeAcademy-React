    ...
    <div className='max-w-full md:max-w-[1024px] lg:max-w-[1066px] p-4 lg:p-6 mx-auto text-[#685f78] dark:text-[#B9B7C0] mt-24 mb-10 md:mt-32 md:mb-20 flex flex-wrap-reverse md:flex-wrap justify-center md:justify-between items-start gap-y-10 border shadow-md'>
      ...
      <div className='space-y-6 md:space-y-10 w-full md:w-[70%] '>
        <div className='space-y-3'>
          <p>Giảng viên</p>
          <h1 className='text-4xl text-gray-800 dark:text-gray-200 font-title pb-2 md:pb-1'>
            {data?.teacher.fullname}
          </h1>
          <b className='text-gray-700 dark:text-gray-300'>{data?.teacher.email}</b>
        </div>
        <div className='flex items-center gap-10'>
          <div>
            <p>Tổng khóa học</p>
            <b className='text-xl text-gray-700 dark:text-gray-300'>{data?.teacher.total_course}</b>
          </div>
          <div>
            <p>Đánh giá</p>
            <b className='text-xl text-gray-700 dark:text-gray-300'>{data?.teacher.count_review}</b>
          </div>
        </div>
        ...
        <div className='flex flex-wrap gap-3 items-center justify-between'>
          ...
          {data?.teacher.facebook && (
            <Link
              target='_blank'
              rel='noopener noreferrer'
              to={`${data?.teacher.facebook}`}
              className='flex items-center justify-center py-2 gap-2 hover:text-white hover:bg-[#f84563] rounded-md border border-[#f84563]   bg-white  text-[#f84563]  w-[45%] md:w-full'
            >
              <Facebook /> <span>Facebook</span>
            </Link>
          )}
          {data?.teacher.linkedin && (
            <Link
              target='_blank'
              rel='noopener noreferrer'
              to={`${data?.teacher.linkedin}`}
              className='flex items-center justify-center py-2 gap-2 hover:text-white hover:bg-[#f84563] rounded-md border border-[#f84563]   bg-white  text-[#f84563]  w-[45%] md:w-full'
            >
              <Linkedin /> <span>Linkedin</span>
            </Link>
          )}
          {data?.teacher.youtube && (
            <Link
              target='_blank'
              rel='noopener noreferrer'
              to={`${data?.teacher.youtube}`}
              className='flex items-center justify-center py-2 gap-2 hover:text-white hover:bg-[#f84563] rounded-md border border-[#f84563]   bg-white  text-[#f84563]  w-[45%] md:w-full'
            >
