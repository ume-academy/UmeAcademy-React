...
const FormChapter = ({ courseData, isRefetch }: ChapterProps) => {
      ...
      content: (
        <div className='flex mt-6 justify-center items-center w-full'>
          <Form layout='vertical' className='w-full' form={form} onFinish={handleSubmitChapter}>
            <Form.Item name='name' label={<h2 className=' mr-2'>Chương học:</h2>} rules={[{ required: true }]}>
              <Input
                ref={inputRef}
              ...
              focus:shadow-[0_0_0_2px_rgba(5,145,255,0.1)] focus:bg-[#fafafa]'
                defaultValue={chapter?.id ? `${chapter.name}` : ''}
                placeholder='Vui lòng nhập tên chương'
              />
            </Form.Item>
          </Form>
        </div>
  ...
  const handleChangeDeleteChapter = (chapter: TFormChapter) => {
    Modal.confirm({
      title: 'Vui lòng xác nhận',
      content: (
        <p className='dark:text-[#b9b7c0] text-[#685f78]'>
          Bạn có chắc chắn muốn xóa chương học có tên <span className='font-desc'>"{chapter.name}"</span> hay không?
        </p>
      ),
      okText: 'Đồng ý',
      okType: 'danger',
  ...
  const listChapter = (chapter: TChapter, index: number): CollapseProps['items'] => [
    {
      key: `${chapter.id}`,
      label: (
        <div className='flex items-center font-subtitle'>
          <h1 className='mr-1 dark:text-[#b9b7c0]'>Chương {index + 1}:</h1>
          <h1 className='mr-4 dark:text-[#b9b7c0]'>{chapter.name}</h1>
          {!hideCourseFunction && (
            <>
              {/* Modal */}
              <EditFilled
                onClick={() => handleFormChapter({ id: chapter?.id, name: chapter.name })}
                style={{ fontSize: 16, color: `${theme === 'dark' ? '#b9b7c0' : '#1e1e1e'}`, marginRight: '12px' }}
              />
              <DeleteFilled
                onClick={() => handleChangeDeleteChapter(chapter)}
                style={{
                  fontSize: 16,
                  height: '18px',
