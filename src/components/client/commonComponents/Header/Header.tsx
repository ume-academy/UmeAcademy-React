...
import { History, Mail, MailOpen } from 'lucide-react'
...
const Header = () => {
  ...
  const {
    ...
    isLoading,
    isFetching
  } = useGetAllFavoriteCoursesQuery(undefined, {
    refetchOnMountOrArgChange: true,
      ...
      }))),
    {
      ...
      label: (
        <button
          ...
          className='border-none w-full flex justify-start p-0 items-center shadow-none dark:text-[#b9b7c0]'
        >
        ...
        <div className='hidden lg:flex items-center justify-between lg:w-[1280px] h-[80px] mx-auto'>
          ...
          <div className='flex justify-between items-center'>
            {isAuthenticated === true ? (
              <>
                {/* teacher */}
                <button onClick={handleToggle} className='mr-[20px] flex  items-center dark:text-[#b9b7c0] hover'>
                  {isTeacherLayout === true ? 'Học viên' : 'Giảng viên'}
                </button>
                {/* dark mode */}
                <button
                ...
                <div className='flex items-center w-[120px] justify-between mx-[20px]'>
                  ...
                  {/* svg thông báo */}
                  <div
                    className={isTeacherLayout ? `flex w-full justify-center items-center cursor-pointer` : 'cursor-pointer'}
                    onClick={showDrawer}
                  >
                    <div className='relative'>
                      <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    ...
                    <Drawer
                      title={
                        <div className="flex justify-between items-center text-black dark:text-[#b9b7c0]">
                      ...
                      closable={false}
                      open={isHidden}
                      autoFocus={false} // Không tự động focus (thêm border) vào phần còn lại của màn hình
                                ...
                                <List.Item.Meta
                                  title={
                                    <div className='flex items-center gap-x-2'>
                                      {/* mark read */}
                                      {
                                        item.is_read === 0 ? <Mail size={18} className='text-black dark:text-[#b9b7c0]' /> : <MailOpen size={18} className='text-black dark:text-[#b9b7c0]' />
                                      }

                                      <span className='text-black dark:text-[#b9b7c0] font-title'>Hệ thống</span>
                                  ...
                                  description={
                                    <>
                                      <p className={`text-black dark:text-[#b9b7c0] ${item.is_read === 0 ? 'font-subtitle text-[#757575]' : ''}`}>
                                        {item.message}
                                      </p>
                                    </>
                                  }
